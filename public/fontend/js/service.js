(function ($) {
  ...
  function get_latitude_longitude() {
    ...
    $.get(
      ...
      function (data, status) {
        $(data.results).each(function (key, value) {
          $("#prequalification_address").val(place.formatted_address);
          $("#prequalification_latitude").val(value.geometry.location.lat);
          $("#prequalification_longitude").val(value.geometry.location.lng);
        });
      }
    );
  }
